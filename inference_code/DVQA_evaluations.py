...
for a in dvqa_dataset['val']:
    ...
    inputs = processor(images=image, text=prompts, return_tensors="pt", padding=True, truncation=True).to(args.device)
    ...
    ans_file.flush()
    print(f"{i} questions have been processed, predicted answer = {answer}, labeled answer = {labeled_answer}")
    i += 1
