...
def micro_analyse(answer_path):
    ...
    for type, (correct, total) in my_dict.items():
        print(f"question type: {type}: micro accuracy: {correct / total}, total {type} questions = {total}, correct = {correct}")
    for type, errors in RMSE.items():
        print(f"question type: {type}: RMSE: {sum(errors) / len(errors)}")
    print("\n\n")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', type=str, choices=['micro', 'macro'], help='mode')
    # Parse the arguments
